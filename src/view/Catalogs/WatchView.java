    ...
    void getSortedCatalog(ArrayList<Product> catalog){
        ...
        switch (userChoice) {
            case "1", "2":
                ...
                break;
            case "3": catalog.stream().filter(product -> product.category.equals(Watches)).filter(product -> getAvailable(product.available).equals("Товар доступен")).map(product -> product.id + " " + product.title + " " + product.price + " " + getAvailable(product.available)).forEach(System.out::println);
                break;
            case "4": catalog.stream().filter(product -> product.category.equals(Watches)).filter(product -> getAvailable(product.available).equals("Товар не доступен")).map(product -> product.id + " " + product.title + " " + product.price + " " + getAvailable(product.available)).forEach(System.out::println);
                break;
            default:
